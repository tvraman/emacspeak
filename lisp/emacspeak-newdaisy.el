...
(require 'xml) ;;; bundled with Emacs22
...
(defvar emacspeak-daisy-frame-seconds 0.02612219949104336
  "Number of seconds in an MP3  frame.")
...
(defun xml-find-tag-by-id (tree id)
  ...
  (cond
   ((string-equal id (xml-get-attribute tree  'id)) tree)
   (t ; walk children
    (let ((children (xml-node-children tree))
          (current nil)
          (found nil))
...
Clip is the result of parsing element <audio .../> as defined by Daisy 3."
  ...
  (unless
      (string-equal "audio" (xml-node-name clip))
    (error "Invalid audio clip."))
  (let ((src (xml-get-attribute  clip 'src))
        (begin (xml-get-attribute  clip 'clipBegin))
        (end (xml-get-attribute  clip 'clipEnd))
        (first nil)
        (last nil)
        (path nil))
...
Clip is the result of parsing SMIL element <text .../> as used by Daisy 3."
  ...
  (unless
      (string-equal "text" (xml-node-name clip))
    (error "Invalid audio clip."))
  (let* ((src (xml-get-attribute  clip 'src))
         ...
         (path (emacspeak-daisy-resolve-uri relative
                                            emacspeak-daisy-this-book)))
    (emacspeak-we-extract-by-id path fragment)))
...
the clip."
  (let ((audio (xml-get-children clip "audio"))
        (text (xml-get-children clip "text"))
        (seq (xml-get-children  clip "seq")))
    (when (and (not audio) seq)
      (setq audio (xml-get-children seq "audio")))
    (when audio
...
Return buffer that holds the result of playing the content."
  ...
  (unless
      (string-equal "content" (xml-node-name content))
    (error "Invalid content."))
  (let* ((src (xml-get-attribute  content 'src))
         (split (split-string src "#"))
         (relative (first split))
         (fragment (second split))
         (path (emacspeak-daisy-resolve-uri relative
...
(defvar emacspeak-daisy-xml-elements
  '(
   ncx head title docTitle
   text audio content
   navStruct ;;; old ncx navObject ;;; old ncx navLabel
   navLisp navMap navPoint
   navTarget)
  "Daisy XML elements.")
...
(defun  emacspeak-daisy-ncx-handler (ncx)
  ...
  (mapc 'emacspeak-daisy-apply-handler (xml-node-children ncx)))
...
(defun emacspeak-daisy-head-handler (element)
  "Handle head element."
  (declare (special emacspeak-daisy-this-book))
  (let ((title  (xml-get-children element "title")))
    (when title
      (setf (emacspeak-daisy-book-title emacspeak-daisy-this-book)
            (apply #'concat (xml-node-children title)))
...
(defun emacspeak-daisy-navObject-handler (element)
  ...
  (let ((text (xml-get-children element "text"))
        (audio (xml-get-children element "audio"))
        (content (xml-get-children element "content"))
        (start (point)))
    (if text
...
(defun emacspeak-daisy-navPoint-handler (element)
  ...
  (let ((label (xml-get-children element "navLabel"))
        (content (xml-get-children element "content"))
        (nav-points (xml-get-children element "navPoint"))
        (start (point)))
