...
int             SetRate(ClientData, Tcl_Interp *, int,
                        Tcl_Obj * CONST[]);
int             GetRate(ClientData, Tcl_Interp *, int,
                        Tcl_Obj * CONST[]);
int             getTTSVersion(ClientData, Tcl_Interp *, int,
                              Tcl_Obj * CONST[]);
int             showAlsaState(ClientData, Tcl_Interp *, int,
                              Tcl_Obj * CONST[]);
int             Say(ClientData, Tcl_Interp *, int,
                    Tcl_Obj * CONST[]);
int             Stop(ClientData, Tcl_Interp *, int,
                     Tcl_Obj * CONST[]);
int             SpeakingP(ClientData, Tcl_Interp *, int,
                          Tcl_Obj * CONST[]);
int             Synchronize(ClientData, Tcl_Interp *, int,
                            Tcl_Obj * CONST[]);
int             Pause(ClientData, Tcl_Interp *, int,
                      Tcl_Obj * CONST[]);
int             Resume(ClientData, Tcl_Interp *, int,
                       Tcl_Obj * CONST[]);
...
alsa_configure(void)
{
    ...
    err =
        snd_pcm_hw_params_set_format(AHandle, params,
                                     DEFAULT_FORMAT);
    if (err < 0) {
    ...
    err =
        snd_pcm_hw_params_set_rate_near(AHandle, params, &rate,
                                        0);
    assert(err >= 0);
    ...
    if (start_delay <= 0) {
        ...
        start_threshold =
            (snd_pcm_uframes_t) ((double) rate * start_delay /
                                 1000000);
    ...
    if (stop_delay <= 0)
        stop_threshold =
            (snd_pcm_uframes_t) (buffer_size +
                                 (double) rate * stop_delay /
                                 1000000);
    else
        stop_threshold =
            (snd_pcm_uframes_t) ((double) rate * stop_delay /
                                 1000000);
    err =
    ...
    err =
        snd_pcm_sw_params_set_xfer_align(AHandle, swParams,
                                         xfer_align);
    assert(err >= 0);
    ...
    // >
    bits_per_sample =
        snd_pcm_format_physical_width(DEFAULT_FORMAT);
...
suspend(void)
{
    ...
    while ((res = snd_pcm_resume(AHandle)) == -EAGAIN)
        sleep(1);               /* wait until suspend flag is *
                                 * * released */
    if (res < 0) {
...
alsa_init()
{
    ...
    if ((err =
         snd_pcm_open(&AHandle, device, SND_PCM_STREAM_PLAYBACK,
                      0)) < 0) {
...
Atcleci_Init(Tcl_Interp * interp)
{
    ...
    _eciVersion =
        (void (*)(char *)) (unsigned long) dlsym(eciLib,
                                                 "eciVersion");
    _eciGetAvailableLanguages =
        (int (*)(enum ECILanguageDialect *, int *)) (unsigned
                                                     long)
        dlsym(eciLib, "eciGetAvailableLanguages");
    _eciNewEx =
        (void *(*)(enum ECILanguageDialect)) (unsigned long)
        dlsym(eciLib, "eciNewEx");
    _eciDelete =
        (void (*)(void *)) (unsigned long) dlsym(eciLib,
                                                 "eciDelete");
    _eciReset =
        (int (*)(void *)) (unsigned long) dlsym(eciLib,
                                                "eciReset");
    _eciStop =
        (int (*)(void *)) (unsigned long) dlsym(eciLib,
                                                "eciStop");
    ...
    _eciSetOutputBuffer =
        (int (*)(void *, int, short *)) (unsigned long)
    ...
    if (!_eciGetAvailableLanguages) {
        ...
        Tcl_AppendResult(interp,
                         "_eciGetAvailableLanguages undef\n",
                         NULL);
    }
    if (!okay) {
    ...
    if (Tcl_PkgProvide(interp, PACKAGENAME, PACKAGEVERSION) !=
        TCL_OK) {
    ...
    if (aDefaultLanguage == NODEFINEDCODESET) {
        Tcl_AppendResult(interp, "No language found",
                         PACKAGENAME, NULL);
        return TCL_ERROR;
    }
    ...
    if ((_eciSetParam(eciHandle, eciInputType, 1) == -1)
        ...
        Tcl_AppendResult(interp, "Could not initialized tts",
                         NULL);
        _eciDelete(eciHandle);
        return TCL_ERROR;
    }
    ...
    Tcl_CreateObjCommand(interp, "say", Say,
                         (ClientData) eciHandle, TclEciFree);
    Tcl_CreateObjCommand(interp, "synth", Say,
                         (ClientData) eciHandle, NULL);
        ...
        int objc, Tcl_Obj * CONST objv[])
{
    int             rc,
                    ...
                    voice;
    if (objc != 2) {
        Tcl_AppendResult(interp, "Usage: getRate voiceCode  ",
                         TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    if (rc != TCL_OK)
        return rc;
    rate =
        _eciGetVoiceParam(eciHandle, voice, 6 /* eciSpeed */ );
        ...
        int objc, Tcl_Obj * CONST objv[])
{
    ...
    if (objc != 3) {
        Tcl_AppendResult(interp,
                         "Usage: setRate voiceCode speechRate ",
                         TCL_STATIC);
        return TCL_ERROR;
    }
    rc = Tcl_GetIntFromObj(interp, objv[1], &voice);
    ...
    rc = _eciSetVoiceParam(eciHandle, voice, 6 /* eciSpeed */ ,
                           rate);
    if (rc == -1) {
        Tcl_AppendResult(interp, "Could not set rate",
                         TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    int objc, Tcl_Obj * CONST objv[])
{
    ...
    for (i = 1; i < objc; i++) {
        ...
        if (Tcl_StringMatch(txt, "-reset")) {
            _eciReset(eciHandle);
            if ((_eciSetParam(eciHandle, eciInputType, 1) == -1)
                || (_eciSetParam(eciHandle, eciSynthMode, 1) ==
                    -1)
                || (_eciSetParam(eciHandle, eciSampleRate, 1) ==
                    -1)) {
                Tcl_AppendResult(interp,
                                 "Could not re-initialized tts",
                                 NULL);
                return TCL_ERROR;
            }
            ...
            if (dest) {
                ...
                free(dest);
                if (!rc) {
                    Tcl_SetResult(interp, "Internal tts error",
                                  TCL_STATIC);
                    return TCL_ERROR;
                }
            }
    ...
    if (Tcl_StringMatch
        (Tcl_GetStringFromObj(objv[0], NULL), "synth")) {
        rc = _eciSynthesize(eciHandle);
        if (!rc) {
            Tcl_SetResult(interp, "Internal tts synth error",
                          TCL_STATIC);
            return TCL_ERROR;
        }
    }
     ...
     Tcl_Interp * interp, int objc, Tcl_Obj * CONST objv[])
{
    ...
    Tcl_SetResult(interp, "Could not stop synthesis",
                  TCL_STATIC);
    return TCL_ERROR;
}
      ...
      Tcl_Obj * CONST objv[])
{
    if (_eciPause(eciHandle, 1))
        return TCL_OK;
    Tcl_SetResult(interp, "Could not pause synthesis",
                  TCL_STATIC);
    return TCL_ERROR;
}
       ...
       Tcl_Obj * CONST objv[])
{
    if (_eciPause(eciHandle, 0))
        return TCL_OK;
    Tcl_SetResult(interp, "Could not resume synthesis",
                  TCL_STATIC);
    return TCL_ERROR;
}
              ...
              int objc, Tcl_Obj * CONST objv[])
{
    ...
    if (objc != 1) {
        Tcl_AppendResult(interp, "Usage: ttsVersion   ",
                         TCL_STATIC);
        return TCL_ERROR;
    }
    _eciVersion(version);
...
{
    if (objc != 1) {
        Tcl_AppendResult(interp, "Usage: alsaState   ",
                         TCL_STATIC);
        return TCL_ERROR;
    }
