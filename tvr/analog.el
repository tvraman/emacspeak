...
(defvar analog-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "?") 'analog-show-help)
    ...
    (define-key map (kbd "t") 'analog-toggle-timer-and-redisplay)
    map)
  "Keymap for analog mode.")

(defvar analog-file-entry-map
  (let ((map (copy-keymap analog-mode-map)))
    ;(define-key map (kbd "<mouse-2>") 'analog-mouse-find-entry-file)
    (define-key map (kbd "C-m")       'analog-find-entry-file)
    map)
  "Additional mapping for file entries.")

(defvar analog-command-entry-map
  (let ((map (copy-keymap analog-mode-map)))
    ;(define-key map (kbd "<mouse-2>") 'analog-mouse-run-entry-command)
    (define-key map (kbd "C-m")       'analog-run-entry-command)
    map)
