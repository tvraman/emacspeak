...
* Conclusion
  ...
  2. This is not mere idealism. The openness is a key enabler for
     creating  new user journeys that were not  envisioned by the
     system's creators.
  3. Rapid, reliable task completion is the most important metric and
     trumps secondary items such as eye-candy --- the latter only
     leads to bloat as evinced by the HTML experience.
  4. Having a clearly identified user problem when designing systems
     wins --- /usability/ is important, but to  truly matter, the
