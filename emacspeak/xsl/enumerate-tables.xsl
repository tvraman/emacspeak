...
License: GPL
Description: Enumerate all tables in a page, but dont recurse
  ...
  <xsl:template match="/html/body">
    <xsl:element name="body">
      ...
      <xsl:if test="count(//table//table)  &gt; 0">
        <h2>
          <a name="__enumerated_tables" id="__nested_tables"><xsl:value-of select="count(//table//table)"/>
          Nested Tables </a>
        </h2>
        
        <xsl:variable name="i" select="//table//table"/>
        <xsl:for-each select="$all">
          <xsl:if test="set:intersection($i, .)">
            <h2><xsl:element name="a"><xsl:attribute name="href">
                  #src-<xsl:value-of select="generate-id(.)"/>
              </xsl:attribute><xsl:attribute name="name"><xsl:value-of select="generate-id(.)"/></xsl:attribute><em>Table <xsl:value-of select="position()"/> </em></xsl:element><xsl:value-of select="count(./tr)"/> Rows 
        ...
        <h2>
          <a name="__about_enumerated_tables">About This Style</a>
        </h2>
